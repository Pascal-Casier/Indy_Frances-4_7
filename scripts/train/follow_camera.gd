...
func _physics_process(delta: float) -> void:
	...
	# Utiliser directement la direction du train sans la projeter sur le plan horizontal
	var target_direction = -follow_target.global_basis.z
	var desired_basis = Basis.looking_at(target_direction)
	
	global_basis = global_basis.orthonormalized().slerp(desired_basis.orthonormalized(), rotation_damping * delta)
