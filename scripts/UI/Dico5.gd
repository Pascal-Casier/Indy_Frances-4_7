...
const CORRECT_SOUND_EFFECT = preload("res://assets/sounds/sfx/correct_answer.mp3")
const WRONG_SOUND_EFFECT = preload("res://assets/sounds/sfx/incorrect2.ogg")
...
func _ready() -> void:
	...
	Global.note_collected.connect(_on_note_collected)
	...
	for category_name in dictionary.categories.keys():
		...
		for french_word in dictionary.categories[category_name]:
			if dictionary.words.has(french_word):
				...
				word_item.add_button(4, preload("res://assets/images/icon_edit.png"), BUTTON_EDIT)
				word_item.add_button(4, preload("res://assets/images/icon_delete.png"), BUTTON_DELETE)
...
func _on_note_collected(french_words: Dictionary, category: String):
	for fr in french_words:
		add_word_from_game(fr, french_words[fr], category)
#  to add a word from an extern object

func add_word_from_game(french: String, portuguese: String, category: String = DEFAULT_CATEGORY) -> void:
	# Vérifie si le mot existe déjà
	if not dictionary.words.has(french):
		# Crée l'entrée dans le dictionnaire
