...
@export var slam_fall_speed := -55.0       # Vitesse de chute forcée (négatif = vers le bas)
@export var slam_gravity_multiplier := 6.0 # Gravité boostée pendant le slam
@export var slam_damage := 40              # Dégâts infligés aux ennemis à l'impact
@export var slam_radius := 2.5            # Rayon de la zone d'impact au sol
@export var slam_knockback_force := 18.0   # Force d'éjection des ennemis
@onready var slam_shake_power := 0.45      # Intensité du camera shake à l'impact
######################################################
...
func _handle_attack() -> void:
	...
	elif not is_on_floor() and not is_ground_slamming:
		# En l'air → Ground Slam !
		_start_ground_slam()
...
func _start_ground_slam() -> void:
	...
	# Stopper le mouvement horizontal net
	velocity.x = 0.0
	velocity.z = 0.0
	
	# Donner une impulsion vers le bas immédiate
	vertical_velocity = slam_fall_speed
	
	# Jouer l'animation de slam (utilise Jump_Idle comme base,
	# remplace par ton anim dédiée si tu en as une)
	...
	# Petit freeze frame "juice" au déclenchement (sensation de puissance)
	Engine.time_scale = 0.15
	await get_tree().create_timer(0.04).timeout
	Engine.time_scale = 1.0

func _handle_ground_slam(delta: float) -> void:
	if not is_ground_slamming:
		return
	
	# Bloquer le mouvement horizontal pendant le slam
	velocity.x = 0.0
	velocity.z = 0.0
	
	# Gravité boostée pour une chute ultra rapide
	vertical_velocity -= gravity * slam_gravity_multiplier * delta
	velocity.y = vertical_velocity
	
	# Atterrissage détecté
	if is_on_floor() and not slam_landed:
		slam_landed = true
		_on_slam_impact()

func _on_slam_impact() -> void:
	# --- 1. Freeze frame à l'impact ---
	Engine.time_scale = 0.05
	await get_tree().create_timer(0.04 * 0.05).timeout  # ~4 frames réelles
	Engine.time_scale = 1.0
	
	# --- 2. Camera shake ---
	if camroot and camroot.has_method("add_shake"):
		camroot.add_shake(slam_shake_power)
	
	# --- 3. Particules d'impact (poussière/débris) ---
	%sparks.emitting = true
	%smoke.emitting = true
	
	# --- 4. Son d'impact ---
	if hurt_audio_stream_player:
		hurt_audio_stream_player.pitch_scale = 0.5  # Son plus grave = plus lourd
		hurt_audio_stream_player.play()
	
	# --- 5. Dégâts et knockback sur les ennemis dans le rayon ---
	_slam_damage_enemies()
	
	# --- 6. Animation d'atterrissage ---
	if anim_state_machine:
		anim_state_machine.travel("Jump_Land")
...
func _slam_damage_enemies() -> void:
	# Cherche tous les ennemis dans un rayon autour du joueur
	var space_state = get_world_3d().direct_space_state
	var sphere_shape = SphereShape3D.new()
	sphere_shape.radius = slam_radius
	
	var query = PhysicsShapeQueryParameters3D.new()
	query.shape = sphere_shape
	query.transform = global_transform
	query.collision_mask = 4  # Adapte le layer selon tes ennemis
	...
	for result in results:
		...
		if body.is_in_group("Enemy"):
			...
			# Knockback radial depuis le centre du slam
			if body is RigidBody3D or body.has_method("apply_knockback"):
				var knockback_dir = (body.global_position - global_position).normalized()
				knockback_dir.y = 0.5  # Légère composante verticale
				if body.has_method("apply_knockback"):
					body.apply_knockback(knockback_dir * slam_knockback_force)
				elif body is RigidBody3D:
					body.apply_central_impulse(knockback_dir * slam_knockback_force)
##############################################################################
...
func _handle_normal_movement(delta: float, h_rot: float) -> void:
	...
	# Si on est en train de slam, gérer ça en priorité
	if is_ground_slamming:
		_handle_ground_slam(delta)
		move_and_slide()
