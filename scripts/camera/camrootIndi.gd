...
func _input(event):
	...
	if event.is_action_pressed("zoom_in"):
		if spring_arm.spring_length > zoom_max:
			spring_arm.spring_length = lerp(spring_arm.spring_length, spring_arm.spring_length -4, 0.09 )
	if event.is_action_pressed("zoom_out"):
		if spring_arm.spring_length < zoom_min:
			spring_arm.spring_length = lerp(spring_arm.spring_length, spring_arm.spring_length +4, 0.09 )
