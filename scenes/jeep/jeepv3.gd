...
@export_group("Saut")
@export var jump_force: float = 8.0          ## Impulsion verticale appliquée au saut
@export var jump_cooldown: float = 0.4       ## Anti-spam du saut
@export var coyote_time: float = 0.15        ## Marge après avoir quitté le sol
@export var max_vertical_speed_to_jump: float = 2.0  ## Anti-triche : empêche de resauter si déjà en mouvement vertical
...
var mud_zone_count: int = 0   # compteur de zones de boue actives (évite le cumul incorrect de linear_damp)
...
func _physics_process(delta: float) -> void:
	...
	# --- Détection du sol / coyote time ---
	is_grounded = ray_cast_3d.is_colliding()
	if is_grounded:
	...
	# --- Saut ---
	# coyote_timer > 0 : au sol ou juste sorti du sol depuis moins de coyote_time
	# jump_cooldown_timer <= 0 : pas de spam
	# linear_velocity.y < max_vertical_speed_to_jump : anti-triche, empêche de resauter
	#   si la jeep est déjà en train de monter rapidement (ex: rampe + double-tap)
	if Input.is_action_just_pressed("jump") \
	and coyote_timer > 0.0 \
	and jump_cooldown_timer <= 0.0 \
	and ball.linear_velocity.y < max_vertical_speed_to_jump:
